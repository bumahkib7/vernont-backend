...
# Builds with Jib and deploys to AWS Beanstalk
#########################################
...
echo -e "${BLUE}[1/5]${NC} Logging into AWS ECR..."
aws ecr get-login-password --region $REGION | docker login --username AWS --password-stdin $ECR_REPOSITORY
echo -e "${GREEN}✓${NC} ECR login successful"
echo ""

# Step 2: Build with Jib
echo -e "${BLUE}[2/5]${NC} Building Docker image with Jib..."
export JIB_TO_IMAGE="${ECR_REPOSITORY}:${IMAGE_TAG}"

# Get ECR credentials for Jib
...
./gradlew :nexus-api:jib \
    ...
    -Djib.console=plain
echo -e "${GREEN}✓${NC} Image built and pushed: ${JIB_TO_IMAGE}"
echo ""

# Step 3: Create Dockerrun.aws.json
echo -e "${BLUE}[3/5]${NC} Creating Dockerrun.aws.json..."
...
echo -e "${BLUE}[4/5]${NC} Creating new application version..."
...
# Step 5: Wait for environment to be ready, then deploy
echo -e "${BLUE}[5/5]${NC} Deploying to Elastic Beanstalk..."
